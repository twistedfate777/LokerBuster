# Generated by Django 6.0.4 on 2026-04-26 05:48

import django.db.models.deletion
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('scanner', '0001_initial'),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.AddField(
            model_name='jobreport',
            name='user',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='reports', to=settings.AUTH_USER_MODEL),
        ),
        migrations.AddIndex(
            model_name='jobreport',
            index=models.Index(fields=['-created_at'], name='idx_report_created'),
        ),
        migrations.AddIndex(
            model_name='jobreport',
            index=models.Index(fields=['scam_score'], name='idx_report_score'),
        ),
        migrations.AddIndex(
            model_name='jobreport',
            index=models.Index(fields=['company_name'], name='idx_report_company'),
        ),
        migrations.AddIndex(
            model_name='jobreport',
            index=models.Index(fields=['is_scam'], name='idx_report_is_scam'),
        ),
    ]
